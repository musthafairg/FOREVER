...
function addSizeVariant() {
  ...
  const variantIndex = sizeVariants.length;

  const variantDiv = document.createElement("div");
  variantDiv.className = "size-variant mb-3";
  variantDiv.innerHTML = `
        <div class="d-flex align-items-center mb-2">
            <select class="form-select form-select-sm me-2" name="size${variantIndex}" required>
            ...
            <input type="number" class="form-control form-control-sm me-2" name="quantity${variantIndex}" placeholder="Quantity" min="0" required>
            <input type="number" class="form-control form-control-sm me-2" name="price${variantIndex}" placeholder="Price" min="0" step="0.01" required>
            <button type="button" class="btn btn-sm btn-danger" onclick="removeSizeVariant(${variantIndex})">Remove</button>
        </div>
    `;
  sizeContainer.appendChild(variantDiv);
  sizeVariants.push({ size: "", quantity: 0, prize: 0 });
}

function getSizeVariants() {
  let variants = [];
  let variantDivs = document.querySelectorAll(".size-variant");
  let sizeSet = new Set();
...
function removeSizeVariant(index) {
  const sizeContainer = document.getElementById("sizeContainer");
  const variantDivs = sizeContainer.getElementsByClassName("size-variant");
  if (variantDivs[index]) {
    sizeContainer.removeChild(variantDivs[index]);
...
async function validateAndSubmit() {
  ...
  const sizeVariantData = getSizeVariants();

  if (!sizeVariantData) return;

  formData.append("sizeVariants", JSON.stringify(sizeVariantData));
...
function validateForm() {
  ...
  const variants = getSizeVariants();
